...
// isPhoneNumber (s [,eok])            True if string s is a valid Phone Call Number. 
// isInternationalPhoneNumber (s [,eok]) True if string s is a valid international phone number.
// isUSZIPCode (s [,eok])              True if string s is a valid U.S. ZIP code.
// isZIPCode (s [,eok])                True if string s is a valid Chinese ZIP code.
// isStateCode (s [,eok])              True if string s is a valid U.S. Postal Code
// isCNProvinceCode (s [,eok])         True if string s is a valid Chinese Province name
...
// isFieldDate (FIELD, separator[,eok])True if field value arguments form a valid date.
...
// checkProvinceCode (theField)        Check that theField.value is a valid Chinese Province Name
// checkUSZIPCode (theField [,eok])    Check that theField.value is a valid US ZIP code.
// checkZIPCode (theField [,eok])      Check that theField.value is a valid Chinese ZIP code.
// checkUSPhone (theField [,eok])      Check that theField.value is a valid US Phone.
// checkPhone (theField [,eok])        Check that theField.value is a valid Phone Call.
...
// checkFieldDate (dateField, warnstring, emptyOK)
//                                     Check that field values form a valid date.
...
var mPrefix_old = "You did not enter a value into the "
var mSuffix_old = " field. This is a required field. Please enter it now."
var mPrefix = ""
var mSuffix = "????????."
...
var iCNZIPCode = "????????????200233?."
var iUSPhone = "This field must be a 10 digit U.S. phone number (like 415 555 1212). Please reenter it now."
var iPhone = "????????????021-64851673."
var iWorldPhone = "This field must be a valid international phone number. Please reenter it now."
var iSSN = "This field must be a 9 digit U.S. social security number (like 123 45 6789). Please reenter it now."
var iEmail_old = "This field must be a valid email address (like foo@bar.com). Please reenter it now."
var iEmail = "E-mail?????,???."
var iCreditCardPrefix = "This is not a valid "
var iCreditCardSuffix = " credit card number. (Click the link on this form to see a list of sample numbers.) Please reenter it now."
var iDay = "????????1-31?????."
var iMonth = "??????1-12????."
var iYear = "?????4???????1950-2050?,???."
var iDatePrefix_old = "The Day, Month, and Year for "
var iDateSuffix_old = " do not form a valid date.  Please reenter them now."
var iDatePrefix = "??????YYYY-MM-DD??????1950-2050,?????"
var iDateSuffix = ""
...
// 
// isPhoneNumber returns true if string s is a valid Phone Call
//
// NOTE: Strip out any delimiters (spaces, hyphens, parentheses, etc.)
// from string s before calling this function.
//
...
// 
// isUSZIPCode returns true if string s is a valid 
...
// Chinese ZIP code.  Must be 6 digits only.
//
// For explanation of optional argument emptyOK,
// see comments of function isInteger.
...
// isCNProvinceCode (STRING s [, BOOLEAN emptyOK])
// 
// Return true if s is a valid Chinese Province Name
// (abbreviation for state).
//
// For explanation of optional argument emptyOK,
// see comments of function isInteger.

function isCNProvinceCode(s)
{   s=stripWhitespace(s);
	if (isEmpty(s)) 
       if (isCNProvinceCode.arguments.length == 1) return defaultEmptyOK;
       else return (isCNProvinceCode.arguments[1] == true);
    return ( (CNProvinceCodes.indexOf("|"+s+"|") != -1) &&
             (s.indexOf(USStateCodeDelimiter) == -1) )
}
...
{   if (isEmpty(s)) 
    ...
    return ((s.length == 4) && isIntegerInRange(s,1950,2999));
}
...
// isFieldDate (FIELD dateField, STRING separator,[emptyOK])
//
// isFieldDate returns true if FIELD arguments is datetype.
// 
function isFieldDate(dateField,separator)
{
	if (isWhitespace(dateField.value))
		if (isFieldDate.arguments.length == 2) 
			return false;
        	else 
        		return (isFieldDate.arguments[2] == true);
	
	var dateArray=new Array();
	dateValue=stripWhitespace(dateField.value);
...
{   theField.value=stripWhitespace(theField.value);
    ...
    {  theField.value = theField.value.toUpperCase();
       if (!isCNProvinceCode(theField.value, false)) 
          return warnInvalid (theField, iProvinceCode);
       else return true;
    }
}
...
// checkUSZIPCode (TEXTFIELD theField [, BOOLEAN emptyOK==false])
//
...
function checkZIPCode (theField, emptyOK)
{
    ...
    { var normalizedZIP = theField.value;
      if (!isZIPCode(normalizedZIP, false)) 
         return warnInvalid (theField, iCNZIPCode);
	  else
		 return true;
    }
}
...
//
// Check that string theField.value is a valid  Phone Call.
//
// For explanation of optional argument emptyOK,
// see comments of function isInteger.
...
{   // Next line is needed on NN3 to avoid "undefined is not a number" error
	...
	if (OKtoOmitDay&&isEmpty(yearField.value)&&isEmpty(monthField.value)&&isEmpty(dayField.value)) return true;
...
function checkFieldDate(dateField,warnString,emptyOK)
{
    if (checkFieldDate.arguments.length == 2) emptyOK = false;
	if (emptyOK&&isEmpty(dateField.value)) return true;
	if (isFieldDate(dateField,'-',emptyOK)) 
		return true ;
	else
    {	alert (warnString+'??????YYYY-MM-DD,????-,?????1950-2050,???!');
		dateField.select();
		dateField.focus();
	    return false;
	}
...
function ValidAttend(Form)
{
	if (Form.depts.options.selectedIndex==0)
		{
			alert('?????!');
			Form.depts.focus();
			return false;
		}
            if (!checkString (Form.T1, "??"))
		{
			return false;
		}
            if (!checkString (Form.T2, "??!"))
		{
			return false;
		}
	Form.submit();
